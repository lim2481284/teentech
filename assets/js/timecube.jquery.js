(function($){
    $.timeCube = function(el, options){
        ...
        base.init = function(){
        	...
        	$(base.options.data).each(function(index, d){
				...
				if(base.options.showDate){
					base.eventDivs[index].html("<h1>" + d.title + "</h1><p class='timelineDate'>" + monthNames[d.startDate.getUTCMonth()] + " " + d.startDate.getUTCDate() + ", " + d.startDate.getUTCFullYear() + "</p> <p>" + d.description + "</p><br><center><img src='"+d.achievement_img+"' class='achievementImg'/> <p class='achievementLabel'>"+d.achievement_label+"</p></center>");
				} else {
        ...
        var xPoint =  (base.options.transitionSpacing * transitionPoint * -1);
        var yPoint = (Math.abs(transitionPoint) * -1 * (base.timelineWidth / 2));
        var degree = (base.options.transitionAngle * transitionPoint);
				// transitionPoint < -1 ? transitionPoint = -1 : null; //cap at 1 / -1
				// transitionPoint > 1 ? transitionPoint = 1 : null; // cap at 1 / -1
				d.style.webkitTransform = "translate3D(" + xPoint + "px, 0px, " + yPoint + "px) rotateY(" +  degree+ "deg)";
